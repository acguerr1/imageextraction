...
# def merge_bboxes_iou(bboxes_input, iou_threshold=0.5):
#     bboxes = list(bboxes_input) # deepcopy
#     merged_bboxes = []
#     while bboxes:
#         bbox = bboxes.pop(0)
#         i = 0
...
#                 bbox = get_encompassing_bbox(bbox, bboxes.pop(i))
#             else:
#                 i += 1
#         merged_bboxes.append(bbox)
#     return merged_bboxes
...
#         x, y, w, h = cv.boundingRect(contour)
#         aspect_ratio = float(w) / h if h > 0 else 0
#         # Remove contours that are likely horizontal lines
#         if w > line_width_threshold and aspect_ratio > aspect_ratio_threshold:
#             print(aspect_ratio)
#             cv.drawContours(mask, [contour], -1, 0, -1)  # Draw the contour on the mask
...
#     cleaned = cv.bitwise_and(binary, binary, mask=mask)

#     # Invert the cleaned image to get the final result
#     cleaned = cv.bitwise_not(cleaned)
...
#         # Remove contours that are likely vertical lines
#         if h > line_height_threshold and aspect_ratio > aspect_ratio_threshold:
#             cv.drawContours(mask, [contour], -1, 0, -1)  # Draw the contour on the mask
...
#     # Top-left corner
#     mask[:corner_height, :corner_width] = 0
#     # Top-right corner
#     mask[:corner_height, width - corner_width:] = 0
#     # Bottom-left corner
#     mask[height - corner_height:, :corner_width] = 0
#     # Bottom-right corner
#     mask[height - corner_height:, width - corner_width:] = 0

#     # cleaned = cv.bitwise_and(bw,mask=mask)
#     cleaned = cv.bitwise_or(bw, cv.bitwise_not(mask))
#     return cleaned, mask
...
#     largest_bbox = max(bboxes, key=lambda bbox: bbox[2] * bbox[3])
#     x, y, w, h = largest_bbox
#     # cvs(draw_bounding_boxes(bw, [largest_bbox]))
...
#     # Apply the mask to the bounding box area in the bw image
#     cleaned = bw.copy()
#     cleaned[mask == 0] = 255

#     # cvs(cleaned, 'cleaned bounding box')
#     return cleaned, mask
...
# def draw_contours(image, contours):
#         if len(image.shape) == 2:
#           contour_image = cv.cvtColor(image.copy(), cv.COLOR_GRAY2BGR)  # Convert to BGRA if needed
#         cv.drawContours(contour_image, contours, -1, (0, 255, 0, 255), 2)
#         return contour_image
...
#           # cvs(page_image, 'final lines out, blurred and cnts filtered img')

#           # Save the processed image w bbxs on page
...
#     # Return the image as is if bounding_boxes is empty
#     if not bounding_boxes:
...
#     for box in bounding_boxes:
#         if isinstance(box[0], (list, tuple)) and len(box) == 4:
#             # Handle format: [(x1, y1), (x2, y2), (x3, y3), (x4, y4)]
#             x1, y1 = box[0]
#             x2, y2 = box[2]
#             w = x2 - x1
#             h = y2 - y1
#         elif isinstance(box[0], tuple) and len(box) == 2:
#             # Handle format: ((x1, y1), (x2, y2))
#             x1, y1 = box[0]
#             x2, y2 = box[1]
#             w = x2 - x1
#             h = y2 - y1
...
#         #words and lines shorter than 1/2 of imgw
#         # if h < height and aspr_actual < float(aspr) and w < imw / 2: # maybe get these values from the first text extraction stats
#         if h > height : #and w < imw / 2: # maybe get these values from the first text extraction stats
